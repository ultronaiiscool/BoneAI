...
**Voice · Beta → Open Free Browser Voice** opens a localhost page that waits for your wake word and sends the following command to BoneAI. **Automatic** tries on-device recognition first if Quest Browser exposes it, then its browser speech service, then configured Groq and Cloudflare free-tier transcription. The latter two require your own keys and have provider limits; their audio leaves the headset. **Open On-Device Voice** requests local-only mode. A one-time browser model download may be needed. Typed commands remain available if speech fails. Browser support and background behavior depend on Quest Browser. Keep the page open.
