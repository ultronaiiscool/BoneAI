...
6. Start BONELAB and open **BoneMenu → BoneAI**.

Do not start the Python helper yourself. BoneAI launches it automatically.

To use Claude, Grok, DeepSeek, or OpenRouter instead, set the provider's documented API key as a Windows environment variable, choose the provider in BoneMenu, and reconnect. See `README.md` for exact variable names. Local Ollama needs no secret.
