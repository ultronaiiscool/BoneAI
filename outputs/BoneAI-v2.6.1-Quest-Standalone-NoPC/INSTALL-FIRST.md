...
7. On Quest choose **Quest Standalone OpenAI**, enter your API key in-game, and reconnect. The key is kept only for that game session.
...
To use OpenAI, Claude, Grok, DeepSeek, or OpenRouter, choose it under **Preferences → BoneAI → AI Provider**, enter the provider key in **API Key (session only)**, and reconnect. Local Ollama needs no secret but is normally useful only on PCVR.
